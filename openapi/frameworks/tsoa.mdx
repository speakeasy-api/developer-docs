---
title: Automatically output OpenAPI from TypeScript applications with tsoa
description: "Build powerful TypeScript APIs, export OpenAPI with tsoa, and use Speakeasy to generate client SDKs."
---

import { Callout } from "@/mdx/components";

# Automatically output OpenAPI from TypeScript applications with tsoa

Anyone who has worked with OpenAPI specifications knows how useful they can be for documenting and sharing APIs. However, it can be a daunting task to create and maintain OpenAPI documents manually, especially as APIs evolve over time. Writing loads of OpenAPI by hand can be tedious and error-prone, but for years the only alternative was littering your codebase with annotations. Fortunately, with [tsoa (TypeScript OpenAPI)](https://tsoa-community.github.io/docs/introduction.html), you can write clean TypeScript code and generate OpenAPI specifications automatically.

tsoa is a particularly clever web application framework, in that it leverages TypeScript's type system not only to help build the API, but also to generate the OpenAPI document, meaning a single source of truth for the API. This single source not only powers documentation, but powers runtime validation, contract testing, SDK generation, and anything else there's an [OpenAPI tool](https://openapi.tools/) to do, without needing to create special workflows to keep everything in sync.
...
{
  "name": "speakeasy-tsoa-example",
  "version": "1.0.0",
  "description": "Speakeasy Train Travel tsoa API",
  "author": "Speakeasy Support <support@speakeasy.com>",
  "license": "Apache-2.0"
}
```
...
info:
  title: speakeasy-train-travel-tsoa
  version: 1.0.0
  license:
    name: Apache-2.0
  contact:
