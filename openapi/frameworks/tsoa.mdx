---
title: How To Generate OpenAPI with tsoa
description: "Build powerful TypeScript APIs, export OpenAPI with tsoa, and use Speakeasy to generate client SDKs."
---

import { Callout } from "@/mdx/components";

# How To Generate OpenAPI with tsoa

Writing a whole load of OpenAPI by hand can be tedious and error-prone, but for years the only alternative was littering your codebase with annotations. Fortunately, with [tsoa (TypeScript OpenAPI)](https://tsoa-community.github.io/docs/introduction.html), you can write clean TypeScript code and generate OpenAPI specifications automatically.
...
tsoa is a particularly clever web application framework, in that it leverages TypeScript's type system to not just build the API, but also to generate the OpenAPI document. This creates a single source of truth for the API, that can power validation, testing, documentation, and SDK generation without needing to create special workflows to keep everything in sync.

## Generating OpenAPI with tsoa

There are two ways to generate an OpenAPI spec using tsoa: via the CLI, or programmatically using internal generator functions.

### Generating OpenAPI via CLI
...
This configures tsoa to output the generated OpenAPI spec in the `build` directory and to use OpenAPI v3.0.

### Programmatically generate OpenAPI

To generate an OpenAPI document using the [internal generator functions](https://tsoa-community.github.io/docs/generating.html#programmatic), import `generateSpec` and call this function by passing a spec config of type `ExtendedSpecConfig` from `tsoa`.
...
As of December 2025, tsoa only outputs OpenAPI documents complying with v2.0 or v3.0. v3.1 support is still a work in progress. Seeing as Speakeasy supports OpenAPI v3.0-v3.2, configuring tsoa to generate using v3.0 will work just fine.
...
```yaml filename="build/swagger.yaml"
info:
...
```typescript filename="src/drinks/drink.ts"
/**
 * The type of drink.
 */
...
```typescript filename="src/drinks/drinksController.ts" mark=6:7
@Route("drink")
...
```typescript filename="src/drinks/drinksService.ts"
...
```typescript filename="src/drinks/drinksController.ts" mark=6
@Route("drink")
export class DrinkController extends Controller {
  @Put("{productCode}")
  public async updateDrink(
...
```typescript filename="src/drinks/drinksController.ts" mark=7
@Route("drink")
export class DrinkController extends Controller {
...
```typescript filename="src/drinks/drinksController.ts" mark=5
@Route("drink")
@Tags("drinks", "bar")
export class DrinkController extends Controller {
  @OperationId("updateDrinkNameOrPrice")
...
```typescript filename="src/drinks/drinksController.ts" mark=4:14
@Route("drink")
export class DrinkController extends Controller {
  @Put("{productCode}")
