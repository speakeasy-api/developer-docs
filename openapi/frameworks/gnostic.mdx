...
#### !!steps ApiExploration

Let's take a detailed tour of our API by exploring `bar.yaml`:

```yaml ! bar.yaml !from ./assets/gnostic/bar.yaml
```
...
```yaml ! bar.yaml focus=1 !from ./assets/gnostic/bar.yaml
```
...
```yaml ! bar.yaml focus=3:6 !from ./assets/gnostic/bar.yaml
```

---

#### !!steps ServersDefinition
...
```yaml ! bar.yaml focus=8:10 !from ./assets/gnostic/bar.yaml
```

---

#### !!steps CreateDrinkEndpoint
...
```yaml ! bar.yaml focus=13:27 !from ./assets/gnostic/bar.yaml
```

---

#### !!steps CreateDrinkResponse

Our `/create-drink` endpoint will return a JSON object with the drink's name, description, recipe, and possibly a photo.

```yaml ! bar.yaml focus=28:39 !from ./assets/gnostic/bar.yaml
```

---

#### !!steps GetDrinkEndpoint
...
```docker ! Dockerfile !from ./assets/gnostic/Dockerfile.txt
```

---

#### !!steps SpeakeasyInstall

We'll install Speakeasy using the `install.sh` script from the Speakeasy repository.

```docker ! Dockerfile focus=3 !from ./assets/gnostic/Dockerfile.txt
```

---

#### !!steps ToolInstallation
...
```docker ! Dockerfile focus=5:11 !from ./assets/gnostic/Dockerfile.txt
```

---

#### !!steps ProjectFilesSetup

We'll copy our project files into the container and set the working directory to `/app`.

```docker ! Dockerfile focus=13:14 !from ./assets/gnostic/Dockerfile.txt
```

---

#### !!steps DependencyUpdate

We'll run `go mod tidy` to ensure all dependencies are up to date.

```docker ! Dockerfile focus=16 !from ./assets/gnostic/Dockerfile.txt
```
...
```docker ! Dockerfile focus=18:21 !from ./assets/gnostic/Dockerfile.txt
```
...
```yaml ! buf.gen.yaml !from ./assets/gnostic/buf.gen.yaml
```
...
```yaml ! buf.gen.yaml focus=1:7 !from ./assets/gnostic/buf.gen.yaml
```
...
```yaml ! buf.gen.yaml focus=9:21 !from ./assets/gnostic/buf.gen.yaml
```
