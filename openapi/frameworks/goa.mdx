...
[Goa](https://github.com/goadesign/goa?tab=readme-ov-file) is a package written in the Go language that allows you to define an API in Go syntax using functions from the Goa design language. Goa uses your definition in its `design.go` file to create:
...
<Callout title="SDK Languages" type="info">
  ...
  TypeScript](/docs/sdks/create-client-sdks#language-support).
</Callout>
