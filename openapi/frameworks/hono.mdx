...
Let's add a [Speakeasy extension](https://www.speakeasy.com/docs/speakeasy-reference/extensions) that adds retries to requests from Speakeasy SDKs by adding a top-level `x-speakeasy-retries` schema to the OpenAPI document. We can also override the retry strategy per operation.
...
The Speakeasy Linter uses a [recommended set of rules](/docs/linting/linting#speakeasy-recommended) that you can configure.
...
Speakeasy [transformations](https://www.speakeasy.com/docs/prep-openapi/transformations) are predefined functions that modify the OpenAPI document to improve it for SDK generation. You can use them to remove unused components, filter operations, and format the OpenAPI document.

[Overlays](https://www.speakeasy.com/docs/prep-openapi/overlays/create-overlays) let you create overlay documents that can be merged with an OpenAPI document. This lets you update and use an OpenAPI document without modifying the original OpenAPI document, which is useful when the same OpenAPI document is used in multiple places. Common use cases include adding Speakeasy extensions, adding examples, and hiding internal APIs from a public SDK.
...
Speakeasy also suggests improvements for your SDK, using [Speakeasy Suggest](https://www.speakeasy.com/docs/prep-openapi/maintenance), which is an AI-powered tool in Speakeasy Studio. You can see suggestions by opening the link to your Speakeasy Studio workspace in the terminal:
...
- [Speakeasy Blog](https://www.speakeasy.com/blog): Read articles about OpenAPI, SDK generation, and more including:
