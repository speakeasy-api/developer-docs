...
OpenAPI does not provide a way to explicitly link a webhook with its registration, which is needed to allow users to subscribe to alerts, but you can address this by following the Speakeasy guide to [using webhooks in OpenAPI](/blog/openapi-tips-webhooks-callbacks#creating-a-webhook-in-openapi).
