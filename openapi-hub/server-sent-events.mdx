...
## SDK integration with flat response handling

When generating SDKs with Speakeasy, server-sent events benefit from flat response handling by default. This feature hoists the `data` field content to the top level, simplifying data access for developers:

```typescript
// Generated SDK with flat response handling (default)
for await (const event of response.stockStream) {
  console.log(`Stock Update: ${event.symbol} is now ${event.price}`);
}

// Without flat response handling
for await (const event of response.stockStream) {
  console.log(`Stock Update: ${event.data.symbol} is now ${event.data.price}`);
}
```

This behavior can be configured in the `gen.yaml` file using the `sseFlatResponse` option for TypeScript and Python SDKs. For more information about configuring server-sent events in generated SDKs, see the [server-sent events documentation](/docs/customize/runtime/server-sent-events).
