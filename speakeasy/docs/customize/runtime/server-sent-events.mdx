...
### Flat response configuration

By default, Speakeasy SDKs use flat response handling for server-sent events, which hoists the `data` field content to the top level. This eliminates the need to destructure the `data` field from each yielded item:

```typescript
// With sseFlatResponse: true (default)
for await (const event of response.chatStream) {
  const { content, model, created } = event; // Direct access to data fields
  process.stdout.write(content);
}

// With sseFlatResponse: false
for await (const event of response.chatStream) {
  const { content, model, created } = event.data; // Must access via data field
  process.stdout.write(content);
}
```

You can control this behavior using the `sseFlatResponse` configuration in your `gen.yaml` file:

```yaml
typescript:
  sseFlatResponse: true # Default: hoists data to top level
