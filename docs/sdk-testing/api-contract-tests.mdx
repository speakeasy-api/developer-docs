...
Speakeasy's implementation currently only supports `simple` criteria and the equality (`==`) and inequality (`!=`) operators for comparing values, and testing status codes, response headers and response bodies.
