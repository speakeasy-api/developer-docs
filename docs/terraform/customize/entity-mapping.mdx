...
This is particularly useful for APIs that support partial updates (PATCH semantics) where sending unchanged fields may cause unintended side effects or where bandwidth efficiency is important.
...
The `create,update` entity operation uses the `only-send-changed-attributes` patch style. During update operations, the generated Terraform provider will compare each attribute against its prior state and only include attributes that have changed in the API request.
