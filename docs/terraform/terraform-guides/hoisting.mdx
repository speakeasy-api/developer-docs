...
By applying the [`x-speakeasy-entity` annotation](/docs/terraform/customize-terraform/entity-mapping), we direct the schema generation process to consider the `Drink` schema as the root of the type schema, effectively flattening the response structure for easier access.
