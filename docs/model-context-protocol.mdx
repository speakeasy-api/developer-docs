...
To turn on MCP server generation, set `enableMCPServer` to true in the `gen.yaml` file and regenerate the SDK on Github or by executing `speakeasy run` locally
