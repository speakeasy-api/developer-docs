...
```typescript
// Example usage of the Code Samples API
import { SpeakeasyCodeSamplesCore } from "code-samples-library";
...
async function fetchCodeSample() {
  const sample = await speakeasyCodeSamples.getCodeSample({
