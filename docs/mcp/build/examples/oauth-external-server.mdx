...
Next, we'll document our OAuth configuration in the OpenAPI document, you can read more about how Gram uses the OpenAPI document [here](/docs/mcp/reference/concepts/tool-sources).
...
After [uploading our OpenAPI document](/docs/mcp/build/toolsets/create-default-toolset) and creating a toolset, we'll go to the playground to test it. When we ask the AI agent to get deployment history, it will automatically detect that OAuth authentication is required:
