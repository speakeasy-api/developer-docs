...
**[Tools](/docs/mcp/reference/concepts/tool-definitions)** are individual callable actions. When uploading a [source](/docs/mcp/reference/concepts/tool-sources) to Gram, API operations and function bundles become tool definitions - a single action that an agent can invoke. For example, a CRM API's OpenAPI document might generate tools like `search_users`, `create_deal`, and `list_pipelines`, while a Gram Function could define custom workflow tools.

**[Toolsets](/docs/mcp/reference/concepts/toolsets)** are curated bundles of tools organized around specific use cases or workflows. Rather than giving an agent access to every available tool, select only the ones needed to accomplish particular tasks. Think of toolsets as specialized toolkits — bring only the necessary tools for each job.
