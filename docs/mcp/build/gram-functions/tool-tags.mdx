...
A [tool variation](/docs/mcp/reference/concepts/tool-variations#tags) that sets tags replaces the tags defined here. See [tag-based tool filtering](/docs/mcp/build/hosting/tool-filtering#effective-tag-precedence-rules) for the full precedence rules that decide which tags a tool is filtered by.

## Next steps

- Enable [tag-based tool filtering](/docs/mcp/build/hosting/tool-filtering) on an MCP server.
- Override tags from the dashboard with [tool variations](/docs/mcp/reference/concepts/tool-variations#tags).
