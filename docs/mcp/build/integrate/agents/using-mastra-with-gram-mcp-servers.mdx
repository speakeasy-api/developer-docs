---
title: "How to connect Mastra agents to MCP"
description: "Build a Mastra agent that connects to an MCP server and uses natural language to query your APIs."
sidebar:
  order: 2
---
...
export async function getMcpAgent(): Promise<Agent> {
  if (!mcpAgentInstance) {
    ...
    mcpAgentInstance = new Agent({
      name: "GramMCPAgent",
      instructions:
        "You are a helpful assistant that uses the CanIPushToProd MCP server tools to answer questions about pushing to production.",
...
const processStep = createStep({
  id: "check-push-day",
  description:
    "Check if it's a good day to push to production using MCP server tools",
...
async function main() {
  ...
  if (result.status === "success") {
    ...
    console.error(
      `Workflow failed: ${result.status === "failed" ? result.error : "Unknown error"}\n`,
    );
    process.exit(1);
  }
}
