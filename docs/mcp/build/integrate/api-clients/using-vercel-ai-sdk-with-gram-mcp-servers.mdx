---
title: "How to connect Vercel AI SDK to MCP"
description: "Learn how to connect the Vercel AI SDK to MCP servers, giving your AI applications access to your APIs and tools."
sidebar:
  order: 3
---

The [Vercel AI SDK](https://ai-sdk.dev/) supports remote MCP servers through its experimental MCP client feature. This allows you to give AI models direct access to your tools and APIs by connecting to [Gram-hosted MCP servers](/docs/mcp/getting-started#quickstart-guides).
...
If you already have a Gram MCP server configured, you can skip to [connecting the Vercel AI SDK to your Gram-hosted MCP server](#connecting-the-vercel-ai-sdk-to-your-gram-hosted-mcp-server). For an in-depth guide to how Gram works and to creating a Gram-hosted MCP server, check out our [introduction to Gram](/docs/mcp/introduction).
...
async function main() {
  ...
  try {
    ...
    const tools = await mcpClient.tools();
    console.log(
      `Connected to MCP server with ${Object.keys(tools).length} tools`,
    );
...
Ready to build your own MCP server? [Try Gram today](/book-demo) and see how easy it is to turn any API into agent-ready tools that work with the Vercel AI SDK and all major AI providers.
