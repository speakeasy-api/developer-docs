...
CLI](/docs/mcp/reference/command-line). Once uploaded, a deployment creates tool
...
The `x-gram` extension also supports [response filtering](/docs/mcp/build/toolsets/response-filtering) through the `responseFilterType` property, which helps LLMs process API responses more effectively.

Using the `x-gram` extension is optional. With Gram's [tool variations](/docs/mcp/reference/concepts/tool-variations) feature, you can modify a tool's name and description when curating tools into toolsets. However, it might be worth using the `x-gram` extension to make your OpenAPI document clean, descriptive, and LLM-ready before bringing it into Gram, so your team doesn't need to fix tool names and descriptions later.
...
These tags drive [tag-based tool filtering](/docs/mcp/build/hosting/tool-filtering), which lets MCP clients connect to a focused subset of a server's tools. A tag set through [tool variations](/docs/mcp/reference/concepts/tool-variations#tags) overrides the operation tags; see the [precedence rules](/docs/mcp/build/hosting/tool-filtering#effective-tag-precedence-rules) for details.
