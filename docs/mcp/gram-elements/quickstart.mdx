...
<Callout type="info">Your package manager may show peer dependency warnings for React packages. These warnings are safe to ignore when using only `/server` exports, as React dependencies are marked as optional.</Callout>
...
For complete configuration options and TypeScript type definitions, see the [API documentation](/docs/mcp/gram-elements/api-reference/interfaces/elements-config).
...
To create your own plugins, see the comprehensive [Plugin Development Guide](/docs/mcp/gram-elements/plugins). The guide covers:
...
<Callout type="info">React 16 and React 17 are not regularly tested. If you run into any issues using Elements with these versions, please reach out to us for support.</Callout>
