...
<Callout title="Looking for simpler authentication?" type="info">
For private servers without external OAuth requirements, consider [Gram
OAuth](/docs/mcp/secure/public-private-servers#gram-oauth) instead. Gram OAuth
uses Gram as the OAuth provider with automatic authentication flows and
organization-based access control. The OAuth Proxy documented on this page is
for external APIs that don't support DCR.
</Callout>
