...
<Table
  data={[
    { dataPoint: "`CustomerID`", description: "A unique string identifying a specific customer account" },
    { dataPoint: "`WorkspaceID`", description: "A unique string identifying a specific customer workspace" },
    { dataPoint: "`Language`", description: "The name of the target language, for example, `go`, `python`, or `typescript`" },
    { dataPoint: "`Template`", description: "The name of the template folder to use for the target, for example, `go`, `typescriptv2`, or `javav2`" },
    { dataPoint: "`RunLocation`", description: "Whether the generation was run in a terminal (`cli`) or GitHub Action (`action`)" },
    { dataPoint: "`GenVersion`", description: "The specific generator version used" },
    { dataPoint: "`CLIVersion`", description: "The specific CLI version used" },
    { dataPoint: "`FeatureTracking`", description: "A list of features the generator does or does not use" },
    { dataPoint: "`ConfigTracking`", description: "A list of configuration values the generator uses" },
    { dataPoint: "`GenIgnoreUsed`", description: "Whether a generation action used a `.genignore` file (read more in the [`.genignore` docs](/docs/customize/code/monkey-patching#mark-files-with-genignore))" }
  ]}
  columns={[
