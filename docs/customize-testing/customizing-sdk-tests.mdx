...
Note that for tests to actually be generated during [`speakeasy run`](/docs/speakeasy-reference/cli/run), the `generateTests` setting must also be enabled. Both settings can be enabled by running [`speakeasy configure tests`](/docs/customize-testing/bootstrapping-test-generation#enabling-test-generation).
