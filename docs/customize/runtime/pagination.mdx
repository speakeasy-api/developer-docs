...
<Table
  data={[
    ...
    {
      type: "`offset`",
      description: "The variable that will be incremented by the number of results returned by the previous execution. **Note:** Requires `outputs.Results`."
    },
    {
      type: "`limit`",
      description: "When provided, `next()` returns `null` (or equivalent) when the number of results returned by the previous execution is less than the value provided."
    }
  ]}
  columns={[
...
<Table
  data={[
    {
      key: "`numPages`",
      description: "When provided, `next()` returns `null` if the `page` input value exceeds the value found at the provided JSON path. **Note:** Requires `page` input."
    },
    {
      key: "`results`",
      description: "When provided, `next()` returns `null` if the array found at the provided JSON path is empty. **Note:** Required by `offset` input."
    },
    {
      key: "`nextCursor`",
      description: "Populates `cursor` with the value found at the provided JSON path when calling `next()`. **Note:** Required by `type: cursor`."
    }
  ]}
  columns={[
    { key: "key", header: "Key" },
