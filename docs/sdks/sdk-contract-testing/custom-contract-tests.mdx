---
title: "Configure Custom Contract Tests"
description: "Learn how to generate custom contracttests using the Arazzo Specification."
---
...
We highly recommend fully setting up [SDK tests](/docs/sdk-contract-testing) in all your SDK repositories before exploring custom contract tests.
...
Before generating tests, ensure that you meet the [testing feature prerequisites](/docs/sdk-contract-testing#prerequisites).
  ...
  tabs={[
    {
      label: "TypeScript",
      language: "typescript",
      code: `// src/__tests__/sdk.test.ts
  ...
  module OpenApiSDK
    class TestSDK < Minitest::Test
      ...
      def test_sdk_user_lifecycle
        ...
        req = Models::Shared::BaseUser.new(
          email: "Trystan_Crooks@hotmail.com",
          first_name: "Trystan",
          last_name: "Crooks",
          age: 32,
          postal_code: "94110",
          metadata: Models::Shared::Metadata.new(
            allergies: "none",
        ...
        assert_equal(32, get_res.user.age)
        refute_nil(get_res.user.postal_code)
        assert_equal("94110", get_res.user.postal_code)

        user = get_res.user
        user.postal_code = "94107"
        user.age = get_res.user.age

        update_res = s.update_user(id: create_res.user.id, user: user)
        ...
        assert_equal(32, update_res.user.age)
        ...
        assert_equal(33, update_again_res.user.age)
