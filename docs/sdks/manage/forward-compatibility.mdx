...
<Table
  data={[
    ...
    },
    {
      ...
      status: "✅",
      impact:
        "**Safe.** With `x-speakeasy-unknown-values: allow`, SDKs handle new values gracefully.",
    },
    {
      change: "New enum value (closed enum)",
      status: "❌",
      impact:
        "**Breaking.** Convert to open enum using `x-speakeasy-unknown-values: allow`.",
    },
    {
      change: "Type change",
      status: "❌",
...
<CodeWithTabs
  ...
  generateOptionalUnionAccessors: true`,
    },
    {
      label: "Python",
      language: "yaml",
      code: `# Smart unions and lax coercion are built-in via Pydantic
# forwardCompatibleEnumsByDefault coming soon`,
    },
  ]}
/>
...
Individual enums and unions can override global defaults using `x-speakeasy-unknown-values: allow` or `x-speakeasy-unknown-values: disallow` in the OpenAPI spec. See the [TypeScript configuration reference](/docs/speakeasy-reference/generation/ts-config#forward-compatibility) for all available options.
</Callout>
