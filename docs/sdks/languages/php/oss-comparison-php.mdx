...
## History of PHP's OpenAPI generators
...
[JanePHP](https://github.com/janephp/janephp) arrived as a dedicated PHP SDK generator with a much better focus on modern PHP features. Sadly it too [lacks support for OpenAPI v3.1](https://github.com/janephp/janephp/issues/759 ) and above, and [does not support enums](https://github.com/janephp/janephp/issues/615) natively.
...
Before creating an SDK its a good idea to validate the files, and whilst there are many tools to do that its best to use the built-in validation functionality of the SDK generator to make sure it's not only valid, but will work well in that tool.
...
Right, onto the SDK generation. Let's create one with OpenAPI Generator and one with Speakeasy, then compare the results.
