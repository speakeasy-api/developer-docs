  ...
  data={[
    {
      ...
      notes:
        "Use granular access tokens with read and write permissions. Classic tokens are deprecated. Supports both SDKs and MCP servers.",
    },
    {
      language: "Python",
      manager: "PyPI",
      prerequisites:
        "[API tokens](https://pypi.org/help/#apitoken) or [Trusted publishing](https://docs.pypi.org/trusted-publishers/)",
      notes:
        "Trusted publishing with OIDC is recommended. See the [setup instructions](#python-pypi-trusted-publishing) below.",
    },
    {
      language: "Go",
      manager: "GitHub",
      ...
      notes:
        "For private repos, add the [Speakeasy app](https://github.com/marketplace/speakeasy-api).",
    },
    {
      language: "Java",
      manager: "Maven",
      prerequisites:
        "[Get started with Maven Central](https://central.sonatype.org/publish-ea/publish-ea-guide/) and [verify domain ownership](https://central.sonatype.org/faq/verify-ownership/)",
      notes: "",
    },
    {
      language: "PHP",
      manager: "Packagist",
      prerequisites: "[See publishing packages](https://packagist.org/)",
      notes:
    ...
    },
    {
      ...
      manager: "NuGet",
      prerequisites:
        "[API key](https://learn.microsoft.com/en-us/nuget/nuget-org/publish-a-package#create-an-api-key)",
      notes: "",
    },
    {
      language: "Terraform",
      manager: "Terraform",
      prerequisites:
        "[GPG Key](https://developer.hashicorp.com/terraform/registry/providers/publishing#preparing-and-adding-a-signing-key)",
      notes: "",
    },
    {
      language: "MCP Server (TypeScript)",
      manager: "npm",
      prerequisites:
        "[Granular access tokens](https://docs.npmjs.com/creating-and-viewing-access-tokens)",
      notes:
        "MCP servers are published as npm packages. Use granular access tokens with read and write permissions.",
    },
    {
      language: "CLI (Alpha)",
      manager: "GoReleaser + GitHub Releases",
      prerequisites: "[Enterprise plan](https://www.speakeasy.com/pricing)",
      notes:
        "CLI targets use generated GoReleaser configuration for cross-platform binary builds rather than `speakeasy configure publishing`. Distribution currently centers on GitHub Releases, generated install scripts, optional Homebrew tap publishing, optional WinGet publishing, and optional Linux package artifacts via nFPM when enabled. [Learn more](/docs/cli-generation/distribute-cli).",
    },
  ]}
  columns={[
  ...
  data={[
    {
      language: "TypeScript",
      manager: "npm",
      example: "@npm-org/package-name",
      convention: "Lower case, hyphen separated",
    },
    {
      language: "Python",
      manager: "PyPI",
      example: "package_name",
      convention: "Lower case, underscore separated",
    },
    {
      language: "Go",
      manager: "GitHub",
      example: "github.com/org/repo-name",
      convention: "Lower case, hyphen separated",
    },
    {
      language: "Java",
    ...
    },
    {
      ...
      manager: "NuGet",
      example: "PackageName",
      convention: "Pascal case, period separated",
    },
    {
      language: "Terraform",
      manager: "Terraform",
      example: "terraform-provider-name",
      convention: "Lower case, hyphen separated",
    },
    {
      language: "CLI",
      manager: "GitHub Releases",
      example: "my-api-cli",
      convention:
        "Lower case, hyphen separated (derived from `cliName` config)",
    },
  ]}
  columns={[
...
<Callout title="Tip">
  For global package managers like PyPI and Maven, ensure the package name is
  unique.
</Callout>
...
<Callout title="Note">
  For more details on `additionalPackageJSON` configuration options, see the
  [TypeScript configuration
  reference](/docs/speakeasy-reference/generation/ts-config#additional-json-package).
</Callout>
...
<Callout title="Important">
  Granular tokens expire after the configured period (default 7 days). For CI/CD
  workflows, set a longer expiration period (up to 1 year) and establish a
  process to rotate tokens before they expire to avoid publishing failures.
</Callout>
...
<Callout title="Tip">
  After verifying that trusted publishing works, remove the `PYPI_TOKEN` secret
  from the repository settings to reduce the attack surface.
</Callout>
...
<Callout title="Tip">
  It's safe to edit the `releaseNotes` property before merging an auto-generated
  PR. The SDK generation workflow validates for changes before running, so your
  edits won't be overwritten during regeneration.
</Callout>
