...
If you have a Swagger 2.0 document, Speakeasy automatically detects and converts it to OpenAPI 3.x when running a workflow. You can also convert manually using the CLI with `speakeasy openapi transform convert-swagger`. See [Transformations](/docs/sdks/prep-openapi/transformations#convert-swagger-to-openapi) for details.
...
A [range of Speakeasy extensions](/docs/sdks/customize/basics) help prepare OpenAPI documents for code generation. Some of the most commonly used extensions are described below.
