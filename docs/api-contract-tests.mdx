...
Before generating tests, ensure that you meet the [testing feature prerequisites](/docs/testing#prerequisites).
...
Speakeasy's implementation currently only supports `simple` criteria and the use of the equality operator `==` for comparing values and for testing status codes, response headers, and response bodies.
...
The `x-speakeasy-test-security` extension enables the use of static values or values pulled from the environment for instantiating an SDK instance and making requests to the API.
