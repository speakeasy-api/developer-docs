...
On the new workflow creation screen if shown a prompt input screen you can prompt Vellum with `Create entrypoint, agent, and output nodes` otherwise use the **Create Workflow** button to create a new workflow.
...
To add a prompt, go to the **Overview** tab and click **Edit Prompt**.
...
Connect the Entrypoint node to the Agent node, then connect the Agent node to the Output. Click on the Output node and under **Overview → Output Value**, select `Agent` for the **Node Output**, and set the **Output Name** to `Text`.
