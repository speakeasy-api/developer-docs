...
Managed authentication lets you store API credentials centrally in Gram [environments](/docs/gram/concepts/environments), so users only need a Gram API key to access your server. This is perfect for demos, trials, or team scenarios where you want to provide easy access without exposing your actual API keys.
...
Before using managed authentication, use the [Playground](/docs/gram/build-mcp/test-toolsets#_top) to ensure your environment is properly configured with all required API keys. The Playground will automatically prompt you to create any missing keys with the correct names.
