---
title: OpenAI Apps SDK with Gram Functions
...
This walkthrough shows how to build and deploy an interactive ChatGPT App using the OpenAI Apps SDK backed
...
If you’re new to Apps, the official
[Apps SDK docs](https://openai.com/index/introducing-apps-in-chatgpt/)
provides a great overview of how connectors show up inside the product. You can
view the full code for this pizza example starting at the pizza-app-gram 
...
- Gram CLI - install via the [official guide](/docs/gram/command-line/installation)
...
deployment simple we'll inline everything into a static blob the MCP server can
...
without making additional network calls. Re-run the script whenever you touch
the UI.

## Build the MCP server

All of the interesting Apps SDK wiring happens inside
`pizzaz_server_node/src/mcp-server.ts`. The sample already sets up everything you
...
Because `withGram` speaks MCP over stdio automatically, you don't have to write
an HTTP transport or worry about session stickiness. Gram handles the hosting,
authentication, and scaling for you once you push the bundle.
