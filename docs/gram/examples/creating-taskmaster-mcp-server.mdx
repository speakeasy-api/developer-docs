...
2. Replace the contents of `gram.ts` with the following code:
```ts
...
4. In the Gram dashboard, go to **Toolsets**, click on the **Taskmaster** toolset, click **Add Tool**, select the newly created function, and save the changes.

   ![Screenshot showing the Add Tool dialog with the send_email function selected](./assets/add-function-to-toolset.png)

5. Update the environment variables by going to the **Auth** tab and clicking on the **+ ADD NEW** button.

   ![Screenshot of the Auth tab showing the Add New button for environment variables](./assets/add-new-environment-variable.png)

6. Then, select the correct environment and add a new variable `RESEND_API_KEY` with your Resend API key, and save the environment.
...
You can now ask their AI assistant to "send a reminder email about the 'Complete documentation' task to user@example.com".
