...
![Screenshot of the Mastra project setup prompts](./assets/mastra-project-setup.png)

After the project is created, open `.env.local` and add your API keys:
```txt
...
This follows Mastra's recommended organization: the `mastra/` directory contains agents and workflows, configuration lives in `config/`, and the entry point orchestrates everything.
...
import dotenv from "dotenv";
import { resolve } from "path";
import { mcpWorkflow } from "./workflows/mcp-workflows";
...
Environment variables load at the top level, so they're available when other modules import them. The `main()` function takes input from command-line arguments or uses a default question, creates a workflow run with `createRunAsync()`, and executes it with `run.start()`. The result object contains either the output on success or an error on failure. The final `if` statement ensures `main()` only runs when the file is executed directly, not when imported as a module.
...
Here's what happens when the workflow runs: the entry point loads environment variables and creates a workflow run. The workflow step gets the agent, which triggers the MCP client to connect to the server and fetch available tools. The agent analyzes the question and calls the appropriate MCP tool (like `can_i_push_to_prod`). The tool runs on the server and returns a result, which the agent uses to generate a natural-language response.
...
- Verify `GRAM_KEY` is set correctly in `.env.local` (check for typos, extra spaces, or missing quotes).
- Confirm the MCP server URL is correct and the server is accessible.
...
- See the [Mastra model documentation](https://mastra.ai/docs/llms/overview) for compatibility details.
