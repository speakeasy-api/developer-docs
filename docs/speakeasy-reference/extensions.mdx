...
<Table 
  data={[
    ...
    { extension: "x-speakeasy-enum-descriptions", description: "Add descriptions to enum values that appear as JSDoc comments in generated SDK code, providing IDE hints and documentation. Supports both array and map formats." },
    ...
    { extension: "x-speakeasy-base64-input-mode", description: "Python only. Opt a JSON string field (`format: byte` or `contentEncoding: base64`) into accepting binary IO or filesystem paths in addition to a pre-encoded base64 string. The SDK base64-encodes non-string inputs before sending. See [Base64 JSON file inputs](/docs/sdks/customize/runtime/base64-json-inputs)." }
  ]}
  columns={[
...
<Table 
  data={[
    ...
    { extension: "x-speakeasy-entity-version", description: "Specify the version of a Terraform resource to support state migrations for breaking schema type changes. Use sparingly - not needed for adding/removing attributes." },
