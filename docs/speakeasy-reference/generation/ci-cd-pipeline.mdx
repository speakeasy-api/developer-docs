...
<Table
  data={[
    ...
    },
    {
      name: "mode",
      description:
        "Workflow mode: `direct` commits changes directly to the branch and publishes SDKs to configured package registries in the same run, `pr` creates a pull request (publishing is handled by the separate `sdk_publish.yaml` workflow when the PR is merged), or `test` fully runs through generation without modifying any GitHub state.",
      type: "string",
      default: "direct",
    },
    {
