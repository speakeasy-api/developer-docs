...
C# SDKs support two formats for specifying additional dependencies:
...
### Extended Format (Package Name with Version and Options)
...
<Table 
  data={[
    { name: "additionalDependencies", required: "false", default: "[]", description: "Add additional dependencies to include in the generated `.csproj` file. Can be specified as simple strings or as objects with package, version, and other NuGet properties." }
  ]}
  columns={[
    { key: "name", header: "Name" },
    { key: "required", header: "Required" },
...
<Table 
  data={[
    { name: "responseFormat", required: "false", default: "envelope-http", description: "Defines how responses are structured. Options: `envelope`, `envelope-http`, or `flat`." },
