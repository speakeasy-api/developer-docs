...
Serve MCP servers from a branded domain, with DNS verification, an optional IP allowlist, a default MCP server at the domain root, and ChatGPT app verification. **Organization settings > Network Access** also hosts [Tailscale private access](/docs/ai-control-plane/distribute/tailscale) for client-facing MCP routes.

### [Logging & Telemetry](/docs/ai-control-plane/org-admin/logging-and-telemetry)

The switches that power [Observe](/docs/ai-control-plane/observe): tool-call logging, skill content, tool I/O capture, session capture, outage behavior, and browser sign-in.
...
Webhook delivery for platform events (beta).
...
The on-device agent that enforces required plugins and MCP configuration across every assistant.
