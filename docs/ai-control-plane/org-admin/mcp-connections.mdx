...
Revoking a connection invalidates the token the Control Plane issued to the client. It does not revoke any upstream OAuth token held for the user through a [remote identity provider](/docs/ai-control-plane/distribute/mcp-servers/authentication/upstream-credentials). The stored upstream credential remains encrypted and becomes effectively inaccessible, but the grant can still appear as an active session in the upstream service until it is revoked there directly.

For how these connections are established, and how session duration and token lifetime differ, see [User sessions](/docs/ai-control-plane/distribute/mcp-servers/authentication/user-sessions).
