...
The **Custom domain** section connects one domain to the organization so MCP servers are served from a branded URL instead of the platform default. Open it from **Organization settings > Network Access** in the dashboard. When private network access is not available, this page is called **Custom Domain**.

For client-facing routes inside a tailnet, see [Tailscale private access](/docs/ai-control-plane/distribute/tailscale). Its private endpoint namespace is pinned when the tailnet is connected; the public custom-domain IP allowlist does not restrict the separate private route.
