...
Remote and tunneled servers can be permissioned by tool name, the same as any other server. Expanding a remote server in the **Specific tools** picker lists the tools recorded for it. A server whose tools have never been recorded has nothing to list, and the picker links to that server's **Inspect** tab instead, where connecting to the live server records them. See [Recording tool metadata](/docs/ai-control-plane/distribute/mcp-servers/remote-servers#recording-tool-metadata).
...
| `mcp:read` | View MCP servers and their configuration | `mcp:connect` |
...
- [Govern agent actions with enterprise identity](/docs/ai-control-plane/guides/govern-agent-actions-with-enterprise-identity) walks the full authorization path from the identity provider to a single MCP tool call, including grant and selector examples.
- The blog post [From your IdP to a tool call: how agent authorization works](/blog/governing-agent-actions-with-enterprise-identity) covers the same path with product context.
