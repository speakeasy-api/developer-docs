...
Cursor Cloud background agents are covered through the **Cursor integration**, not the device agent. Connect Cursor under **Organization settings > Integrations**, and sessions from background agents arrive with the developer's own email address, the conversation id, and token and cost figures. Nothing is installed in the sandbox.

<Callout type="warning">
  Do not deploy the device agent into a Cursor Cloud sandbox to enforce plugins or MCP configuration. It installs and reports success, but nothing in that environment loads what it writes: the agent runs outside the sandbox, and the local plugin directory it targets does not exist there. The result is a device that reports as covered while enforcing nothing.
</Callout>

Enforcement for Cursor on a **developer's own machine** is unaffected: the device agent installs the organization's plugins and MCP servers into Cursor there as normal. See [MDM installations](/docs/ai-control-plane/reference/device-agent/mdm-installations).
