...
MCP servers attach an issuer from their Authentication settings. See [Upstream credentials](/docs/ai-control-plane/distribute/mcp-servers/authentication/upstream-credentials) for how a server references a provider and what an end user sees when connecting one.
