...
Shadow MCP requests can also be resolved from the [Shadow MCP page](/docs/ai-control-plane/secure/shadow-mcp), which approves by adding an allow rule for the whole audience of the policy rather than for the requester alone. See [Shadow MCP allow and block lists](/docs/ai-control-plane/guides/shadow-mcp-allow-and-block-lists) for how the two lists and the allow rules interact.
