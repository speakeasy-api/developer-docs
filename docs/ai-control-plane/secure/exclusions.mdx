...
The fastest way to write an exclusion is from a finding that shouldn't have fired. In [Watchdog](/docs/ai-control-plane/secure/watchdog), select the signals and choose **Suppress > Create Rule**: the criteria prefill from the signal, the matched span where there is one and otherwise the rule or the source. An [agent session](/docs/ai-control-plane/observe/agent-sessions) transcript offers the same exclusion action on a flagged finding.
