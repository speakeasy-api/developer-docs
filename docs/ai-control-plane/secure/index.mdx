...
Create and manage the policies that scan agent sessions. A stepped editor configures what to detect, which sessions are in scope, whether to flag or block, severity, and audience, including natural-language guardrail policies. Exclusion rules and manually dismissed false positives live here as tabs.
...
The catalog of built-in detectors behind guardrail policies (secrets, financial data, PII, healthcare, prompt injection, and more) plus custom rule authoring with CEL expressions, live testing, and AI-suggested rules. Opens from **Secure > Detection Rules** in the dashboard.
