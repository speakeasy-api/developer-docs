...
- OpenAPI documents
- Functions (TypeScript, JavaScript, or Python)
- External MCP servers, added from the [catalog](/docs/ai-control-plane/connect/catalog), registered by URL as custom remote servers, or connected from a private network as [tunneled MCP servers](/docs/ai-control-plane/distribute/mcp-servers/tunneling)
...
A selection of those tools is then served as an
[MCP server](/docs/ai-control-plane/distribute/mcp-servers), tailored to a
specific use case or workflow and installable into LLM clients for automated
usage.

## Functions

Speakeasy Functions are snippets of code that define arbitrary tasks for AI agents
to execute via MCP servers. Unlike OpenAPI-based tools that map to existing
REST API endpoints, Functions can call multiple APIs, connect to remote
...
At its core, a Function is a zip file containing two files:
...
For more details on creating and deploying Functions, see the
[Functions guide](/docs/ai-control-plane/connect/sources/functions).
...
Using the `x-gram` extension is optional. With the [tool variations](/docs/ai-control-plane/reference/concepts/tool-variations) feature, a tool's name and description can be modified when selecting the tools a server exposes. However, it might be worth using the `x-gram` extension to make an OpenAPI document clean, descriptive, and LLM-ready before bringing it into the platform, so the team doesn't need to fix tool names and descriptions later.
...
Existing MCP servers can be brought into the platform as sources alongside OpenAPI documents and Functions:
...
- **Tunneled MCP servers** — MCP servers running inside a private network, connected through an outbound tunnel. See [tunneled MCP servers](/docs/ai-control-plane/distribute/mcp-servers/tunneling).

Tools from external MCP servers deploy like any other source and can be combined with API tools and Functions on a single MCP server.
