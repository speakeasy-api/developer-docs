...
The device agent is a lightweight endpoint agent for developer machines and cloud coding sandboxes. It installs, maintains, and reports on the organization's mandated AI agent configuration: which plugins and MCP servers Claude Code, Codex, Cursor, OpenCode, GitHub Copilot, and OpenClaw should have, kept in place even as tools update or users switch accounts. On laptops it runs as a small background daemon with a menu bar app on macOS, Windows, and Linux, and deploys through MDM or a self-serve installer. In Anthropic-hosted Claude Code on the web it runs inside the session itself, installed by the environment's setup script; see [Claude Code on the web](/docs/ai-control-plane/org-admin/device-agent#claude-code-on-the-web). Cursor Cloud background agents are covered through the [Cursor integration](/docs/ai-control-plane/org-admin/device-agent#cursor-cloud) instead, with nothing installed in the sandbox.
...
- **Claude Code on the web.** Same policy fetch over `app.getgram.ai`. The hosted VM has no MDM; identity is the `org_token` in `/etc/speakeasy/managed.json`, written by the environment's setup script. See [Claude Code on the web](/docs/ai-control-plane/org-admin/device-agent#claude-code-on-the-web).
...
- **Claude Code on the web** - The environment's setup script, generated by the dashboard's **Remote sessions** walkthrough, writes `/etc/speakeasy/managed.json` with a shared identity and an org token, so there is no interactive enroll step in the session. See [Claude Code on the web](/docs/ai-control-plane/org-admin/device-agent#claude-code-on-the-web).
