...
- **Telemetry wiring** - The agent configures Claude Code's OpenTelemetry export to point at the platform's telemetry ingest, seeding the session logs and token metrics behind [Observe](/docs/ai-control-plane/observe). OpenCode has no OpenTelemetry exporter, so there is nothing to wire up there: its observability plugin reports sessions, tool calls, tokens, and cost on its own. See [distributing plugins to OpenCode](/docs/ai-control-plane/distribute/plugins/opencode).
