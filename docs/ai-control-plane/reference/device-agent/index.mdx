---
title: Device Agent
description: "A lightweight endpoint agent that keeps org-mandated AI agent configuration in place on developer machines and in cloud coding sandboxes, and reports coverage back to the platform."
---

import { Callout } from "@/mdx/components";
import DeviceAgentDiagram from "@/components/astro/docs/DeviceAgentDiagram";

The device agent is a lightweight endpoint agent for developer machines and cloud coding sandboxes. It installs, maintains, and reports on the organization's mandated AI agent configuration: which plugins and MCP servers Claude Code, Codex, Cursor, OpenCode, GitHub Copilot, and OpenClaw should have, kept in place even as tools update or users switch accounts. On laptops it runs as a small background daemon with a menu bar app on macOS, Windows, and Linux, and deploys through MDM or a self-serve installer. In a cloud sandbox it runs inside the session itself, installed by the environment's setup script. See [Claude Code on the web](/docs/ai-control-plane/org-admin/device-agent#claude-code-on-the-web) and [Cursor Cloud](/docs/ai-control-plane/org-admin/device-agent#cursor-cloud).
...
The platform enforces policy at MCP tool-call time, but that enforcement only applies if the AI agent on the laptop or in a supported Cloud environment actually routes through a sanctioned MCP server. The device agent is the device-side guarantee that it does. It closes the most common bypass path: configuration drift on developer laptops, including the case where a user switches their AI agent to a personal account and silently loses the org's plugins, telemetry, and guardrails. Cloud sessions have the same gap until their environment is configured.
...
- **Cloud sandboxes.** Same policy fetch over `app.getgram.ai`. A hosted sandbox has no MDM; identity is the `org_token` in `/etc/speakeasy/managed.json`, written by the environment's setup script. See [Claude Code on the web](/docs/ai-control-plane/org-admin/device-agent#claude-code-on-the-web) and [Cursor Cloud](/docs/ai-control-plane/org-admin/device-agent#cursor-cloud).
...
- Self-update applies on **macOS, Windows, and Linux**. On Windows the daemon verifies the newer signed MSI and launches it through a detached Windows Installer process; fleets that prefer MDM-owned version moves set `notify` instead. See [Microsoft Intune](/docs/ai-control-plane/reference/device-agent/mdm-installations/intune#windows-reference).
- `speakeasy update --apply` runs the same download-verify-swap on demand, for scripted or headless use.

## Enrollment

Three enrollment paths cover evaluation, fleet rollout, and cloud sandboxes:
...
- **Cloud sandbox** - The environment's setup script writes `/etc/speakeasy/managed.json` with a shared identity and an org token, so there is no interactive enroll step in the session. For Claude Code on the web the dashboard's **Remote sessions** walkthrough generates that script; for Cursor Cloud it is configured in the repository or Cursor environment. See [Claude Code on the web](/docs/ai-control-plane/org-admin/device-agent#claude-code-on-the-web) and [Cursor Cloud](/docs/ai-control-plane/org-admin/device-agent#cursor-cloud).
