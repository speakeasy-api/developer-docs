...
Windows cannot self-update, because the OS locks running executables, so version moves are an MSI re-push and `auto_update` must stay on `"notify"` rather than `"automatic"` on an MSI fleet. See [Microsoft Intune](/docs/ai-control-plane/reference/device-agent/mdm-installations/intune#windows) for the full recipe, including the detection rule, the `%ProgramData%` ACL grant, and the service-restart requirement.
