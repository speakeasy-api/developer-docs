...
The agent ships as a single Developer ID signed, notarized, universal `.pkg` that installs the daemon, CLI, menu-bar UI, and privileged helper together and registers its own launchd jobs. No script, no separate service-install step. Download the current release from **Organization Settings → Secure → Device Agent** in the dashboard, or fetch it directly from the release bucket at `https://storage.googleapis.com/speakeasy-device-agent-releases-prod/v<version>/speakeasy-agent_<version>.pkg`.

The payload installs machine-wide. The binaries and the **Speakeasy Agent** app land in `/Library/Application Support/Speakeasy/`, the daemon and UI register as LaunchAgents in `/Library/LaunchAgents/`, and the root helper registers as a LaunchDaemon in `/Library/LaunchDaemons/`. The running agent seeds per-user copies under each user's home directory. Where an MDM audits the install, point the rule at the package receipt: `pkgutil --pkg-info com.speakeasy.agent.pkg`.
...
The agent ships a per-machine, Authenticode-signed `.msi` (`speakeasy-agent_<version>.msi`) that installs the daemon, CLI, and UI under `C:\Program Files\Speakeasy\` and registers the daemon as a LocalSystem service. Download it from the release bucket at `https://storage.googleapis.com/speakeasy-device-agent-releases-prod/v<version>/speakeasy-agent_<version>.msi` and push it as a Win32 app or line-of-business MSI. The same bucket carries the raw daemon and CLI binaries (`speakeasyd_<version>_windows_amd64.exe` and `speakeasy_<version>_windows_amd64.exe`) for scripted or air-gapped installs, but they are unsigned. Prefer the MSI.
...
<Callout type="info">
  The coverage states below are attested **per assigned user, not per device**. The agent reports a heartbeat for the enrolled user roughly every minute; a device counts as covered when its MDM-assigned user has a recent heartbeat from *some* device. It proves "this device's user runs the agent," not "this specific device is monitored." Evidence exports carry the stronger per-device claim where it is available: each pushed record states its own strength in `agent_attestation`. See [Export coverage as compliance evidence](#export-coverage-as-compliance-evidence).
</Callout>
