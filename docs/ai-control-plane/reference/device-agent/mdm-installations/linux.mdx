...
The UI ships for amd64 only and does not auto-start on any platform. Download the `speakeasy-ui_<version>_linux_amd64` binary from the release bucket, mark it executable, and add an XDG autostart entry under `~/.config/autostart/` if the fleet wants it. Enforcement does not depend on the UI: the daemon is fully functional without it.
