...
<div
  className="flex w-full flex-wrap items-center justify-center gap-x-10 gap-y-6 md:gap-x-14 lg:gap-x-16"
  style={{ margin: "2rem 0" }}
>
  <a
...
- **Telemetry ingest** - An OTLP endpoint receives logs and telemetry from instrumented agents. Events land in ClickHouse, power the [Observe pages](/docs/ai-control-plane/observe), and can follow project-scoped routes to external collectors through [data exports](/docs/ai-control-plane/observe/opentelemetry).
