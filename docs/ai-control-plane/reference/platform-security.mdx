...
import StackArchitectureDiagram from "@/components/astro/docs/StackArchitectureDiagram";
...
<Callout title="Trust center" type="info">
  For a security and compliance review, visit the [Speakeasy Trust
  Center](https://trust.speakeasy.com/) for audit reports, certifications, and
...
{/* Inline styles undo the docs-wide `img { blog-image }` chrome (gray fill,
    border, rounded corners) so the official marks render as bare images. */}
...
<div style={{ display: "flex", alignItems: "center", gap: "2.5rem", margin: "1.5rem 0" }}>
  <img
    src="/assets/docs/ai-control-plane/soc2-aicpa-badge.webp"
    alt="AICPA SOC for Service Organizations badge"
    style={badgeStyle}
  />
  <img
    src="/assets/docs/ai-control-plane/iso27001-certified-badge.webp"
    alt="ISO 27001 Information Security Management Certified badge"
    style={badgeStyle}
  />
</div>
...
{/* Mirrors the homepage FoundationLogos treatment exactly: same assets,
    monochrome filter, sizes, and centered row layout. `logoStyle` undoes the
    docs-wide `img { blog-image }` chrome so the logos render as bare images. */}
...
<StackArchitectureDiagram client:load />
...
- **Tool connectors** - Turn [sources](/docs/ai-control-plane/connect/sources) into tools. Supported source types include OpenAPI 3.0 and 3.1 documents, TypeScript functions, third-party catalog servers, custom remote MCP servers, and tunneled MCP servers.
- **Telemetry ingest** - An OTLP endpoint receives logs and telemetry from instrumented agents. Events land in ClickHouse, power the [Observe pages](/docs/ai-control-plane/observe), and can be forwarded to an external collector via [OTEL forwarding](/docs/ai-control-plane/org-admin/logging-and-telemetry).
...
![The Logs settings page with storage toggles for logs, skill content, tool I/O, agent session capture, fail-open, and browser sign-in](/assets/docs/ai-control-plane/logs-storage-toggles.webp)

### Can the platform store API secrets?

By default, the platform does not store API secrets.

Customers have the option to store API secrets if they choose to use platform-managed authentication and API keys. Stored secrets live in [Environments](/docs/ai-control-plane/distribute/environments), which are secure, encrypted stores of key-value pairs scoped to the project. When an MCP server uses a managed OAuth flow, End User access and refresh tokens are encrypted before being stored so the platform can complete the flow on the End User's behalf.

### How long is log data retained?

By default, agent session transcripts, tool calls, and metadata are stored for 90 days. Which categories are stored at all is controlled by the **Logs** page toggles shown above; see [Logging & Telemetry](/docs/ai-control-plane/org-admin/logging-and-telemetry) for the full reference.

## Further reading

- [Enterprise support](/support/enterprise)
