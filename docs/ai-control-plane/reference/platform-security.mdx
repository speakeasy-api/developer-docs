...
- **Telemetry ingest** - An OTLP endpoint receives logs and telemetry from instrumented agents. Events land in ClickHouse, power the [Observe pages](/docs/ai-control-plane/observe), and can be forwarded to an external collector via [OTEL forwarding](/docs/ai-control-plane/org-admin/logging-and-telemetry/otel-forwarding).
