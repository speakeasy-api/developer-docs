...
Cost figures are reported by the AI client rather than recomputed by the platform. Claude Code calculates cost as token usage multiplied by published API rates and includes it in the telemetry it emits, and the platform sums those reported values per session and model. In device telemetry, only Anthropic clients report dollar costs. Connected provider integrations fill the gap for other clients: [OpenAI Compliance Logs](/docs/ai-control-plane/org-admin/ai-integrations/openai-compliance) supplies Codex and ChatGPT spend, including Codex cloud surfaces that emit no device telemetry, and the [Cursor integration](/docs/ai-control-plane/org-admin/ai-integrations/cursor) supplies Cursor spend. Sessions from clients that report no cost and have no provider integration contribute $0 to totals, though token counts remain available for every client. There is no fallback pricing table, so usage from an unrecognized model shows only whatever cost its client reported, which is usually nothing.
