...
Speakeasy analyzes tool call results for prompt injection while a skill is active. Scanning requires an enabled Prompt Injection [risk policy](/docs/ai-control-plane/secure/guardrails). Organization administrators can select **"Set up scanning"** or **"View policy"** from the configuration card at the top of the **Skills** page.
