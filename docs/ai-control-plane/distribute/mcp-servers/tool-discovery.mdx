...
On a remote or tunneled server, nothing is stored. Each `tools/list` request is proxied to the upstream and answered from whatever the upstream returns at that moment.
...
<Callout type="warning">
  Tool variations do not apply to remote or tunneled MCP servers. Variations belong to the toolset backend, and a remote-backed server never runs that path. The equivalent on a remote server is tool metadata, which layers annotations and access rules over the live tool list, set from the server's **Tools** tab.
</Callout>
