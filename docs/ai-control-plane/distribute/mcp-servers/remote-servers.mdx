...
Anything not configured automatically can be set up from the server's **Authentication** tab. See [Upstream credentials](/docs/ai-control-plane/distribute/mcp-servers/authentication/upstream-credentials).
...
- **Settings** — display name, upstream URL, upstream headers, visibility, custom slug and domain, publishing, and deletion
- **Authentication** — the identity provider callers authenticate against, and the remote identity providers used to reach the upstream
- **Tools** — annotations and per-tool access, described in [How tools appear on a server](/docs/ai-control-plane/distribute/mcp-servers/tool-discovery)
