...
If you already have an API with OAuth authentication and just want to configure Speakeasy to use it, you can skip directly to [Configuring Speakeasy to use OAuth](#configuring-speakeasy-to-use-oauth).
</Callout>
...
- Finally, integrate with the Speakeasy OAuth configuration and test protected endpoints in the playground
...
Next, we'll document our OAuth configuration in the OpenAPI document, you can read more about how Speakeasy uses the OpenAPI document [here](/docs/ai-control-plane/reference/concepts/tool-sources).
...
## Configuring Speakeasy to use OAuth

Now we'll configure Speakeasy to work with our external OAuth server. After uploading the OpenAPI document, and creating a new toolset, go to the toolset settings page and click **Connect OAuth**.
...
The OAuth Authorization Server Metadata is a JSON object that describes the OAuth server. It's used by Speakeasy to discover the OAuth server's capabilities.
...
## Testing in the Speakeasy playground

Finally, let's test everything in the Speakeasy playground.

After [uploading our OpenAPI document](/docs/ai-control-plane/reference/concepts/toolsets) and creating a toolset, we'll go to the playground to test it. When we ask the AI agent to get deployment history, it will automatically detect that OAuth authentication is required:

![Speakeasy Playground Testing](/assets/docs/gram/img/guides/oauth-external-server/gram-playground-testing.png)
...
![Speakeasy Access Token Required](/assets/docs/gram/img/guides/oauth-external-server/gram-access-token-required.png)

:::note[Note]
In the Speakeasy playground, you need to manually add OAuth tokens to environment variables for testing. In a proper MCP client implementation, the OAuth flow would be handled automatically by the client, including token refresh and management.
:::
...
### Adding the token to Speakeasy

Now we can add our OAuth token to the Authentication environment variables in Speakeasy:

![Speakeasy Playground OAuth Token](/assets/docs/gram/img/guides/oauth-external-server/gram-playground-oauth-token.png)

Once we've added a valid OAuth token, we can test the protected endpoints in the playground:

![Speakeasy OAuth Complete](/assets/docs/gram/img/guides/oauth-external-server/gram-oauth-complete.png)
