...
**Sub catalogs** enable sharing private [catalogs](/docs/ai-control-plane/connect/sources/catalog/overview) with other organizations. This is useful when distributing internal MCP tools to partners, customers, or other teams within a company that operate as separate Speakeasy organizations.

## The peered organization model

Speakeasy uses a **super/sub organization** model to manage cross-organization catalog sharing:
...
Before sharing catalogs, establish a peer relationship with the target organization. In the Speakeasy dashboard:
...
When a sub organization lists available registries, Speakeasy combines:
...
- Learn how to [create catalogs](/docs/ai-control-plane/connect/sources/catalog/overview) to organize toolsets
- Read about [controlling server visibility](/docs/ai-control-plane/distribute/mcp-servers) for fine-grained access control
