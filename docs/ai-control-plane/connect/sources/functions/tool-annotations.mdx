...
description: "Add behavior hints to Speakeasy Functions so AI models and clients can make better decisions about tool invocation."
---
...
The Speakeasy Functions Framework supports MCP tool annotations directly in tool definitions. Add annotations to any tool using the `annotations` property:
...
When no annotations are specified, Speakeasy applies conservative defaults: tools are assumed to be potentially destructive and open-world. Specify annotations explicitly to give clients more accurate information about tool behavior.
...
Annotations defined on Functions are automatically forwarded when tools are exposed through MCP servers. No additional configuration is needed — the annotations set in the tool definition are passed through to MCP clients as part of the tool listing.

## Next steps

- Learn more about the [Functions Framework](/docs/ai-control-plane/connect/sources/functions/functions-framework)
