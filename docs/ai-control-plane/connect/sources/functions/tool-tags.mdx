---
title: "Tool tags"
description: "Tag Speakeasy Functions tools so MCP clients can connect to a focused subset of an MCP server using tag-based tool filtering."
---
...
The Speakeasy Functions Framework supports tags directly in tool definitions. Add tags to any tool using the top-level `tags` property:
...
Tags defined on a Function become the tool's source tags. They are available as filters once tag-based filtering is enabled on the MCP server, and they can be overridden from the dashboard.
