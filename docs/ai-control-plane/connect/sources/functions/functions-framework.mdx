...
description: "Learn how to use the Speakeasy Functions Framework with TypeScript, including context objects and best practices."
---
...
The Speakeasy Functions Framework provides a streamlined way to build MCP tools using TypeScript. It handles MCP implementation while letting you focus on your tool logic.

![Choosing the Functions framework in the project scaffolder](/assets/docs/gram/img/functions/npm_gram-framework.png)
...
- **`annotations`(optional):** Behavior hints for AI models and clients (see [Tool annotations](/docs/ai-control-plane/connect/sources/functions/tool-annotations))
...
Tool calls that arrive over MCP carry attribution about the caller:

- **`ctx.clientInfo`:** The name and version of the calling MCP client, when the client reports them
- **`ctx.oauthClientId`:** The OAuth client ID attributed to the caller, when one accompanied the call
- **`ctx.meta`:** The raw `_meta` block sent with the call, for keys the framework does not model yet

Every field is optional. Calls that do not arrive over MCP, such as direct instance invocations and resource reads, leave all three undefined.
...
<Callout title="Attribution only" type="warning">
  Caller identity is metadata attached to the request, and a tool cannot
  distinguish a value the host attached from one the caller supplied. Use these
  fields for logging, metrics, and telling one integration's traffic from
  another's. Never gate access on them. Authorization belongs to whatever
  verifies credentials in front of the tool, such as
  [OAuth](/docs/ai-control-plane/connect/sources/functions/add-oauth).
</Callout>
...
Functions can access environment variables directly from `process.env`:
...
For more details on configuring and managing environment variables in Functions, see [Configuring environments](/docs/ai-control-plane/connect/sources/functions/configuring-environments).
...
Instances of `Gram` can be composed together using the `.extend()` method, allowing tool definitions to be split across multiple files and modules. This pattern is similar to Hono's grouping pattern and helps organize larger codebases.
...
const gram = new Gram().extend(trainGram).extend(flightGram);
...
const gram = new Gram().extend(weatherGram).extend(newsGram);
...
- [Build and deploy](/docs/ai-control-plane/connect/sources/functions/build-deploy) Functions.
