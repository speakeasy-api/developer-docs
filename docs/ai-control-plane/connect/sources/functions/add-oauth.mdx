---
title: Add OAuth to TypeScript functions
description: Learn how to implement OAuth authentication in TypeScript functions for secure third-party integrations.
---

Integrating OAuth into Speakeasy Functions is a simple process.
...
Speakeasy will then handle the OAuth exchange and automatically supply the token to your function.
...
Configuring OAuth for a toolset containing Functions is the same as configuring OAuth for any other toolset.
You can follow the steps in our [guide](/docs/ai-control-plane/distribute/mcp-servers/secure-with-oauth) to get started.

## Caveats

Only one managed OAuth provider can be attached to an MCP server. However, other security schemes defined in your OpenAPI spec (API keys, bearer tokens, etc.) are still accepted alongside OAuth — users can authenticate with whichever method they prefer. See [Multiple security schemes](/docs/ai-control-plane/distribute/mcp-servers/secure-with-oauth#multiple-security-schemes) for details.
