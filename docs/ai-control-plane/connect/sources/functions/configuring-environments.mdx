...
description: |
  Learn how to configure and use environment variables in Speakeasy Functions.
---
...
You can read environment variables directly from `process.env` in your Functions. Preconfigured values defined in `envSchema` can also be accessed from `ctx.env` in the Functions Framework:
...
You can set up environment variables that end users will provide using `envSchema`. Environment variables specified in the `envSchema` can be provided by MCP users via headers or via stored environments.
...
To attach an environment to your Function, first ensure you have [created an environment](/docs/ai-control-plane/reference/concepts/toolsets) with the configuration values you want to attach.

Then follow these steps:

- Navigate to the **Toolsets** tab.
- Click the **More Actions** (<MoreVertical className="inline" size={16} />) menu on the Source Card for your Function.
...
If you have a Speakeasy API key, you can provide that key and a `GRAM_ENVIRONMENT` header set to the slug for your environment. For an example of using environments with the SDKs, see [the guide to using environments with the Vercel AI SDK](/docs/ai-control-plane/distribute/environments).
