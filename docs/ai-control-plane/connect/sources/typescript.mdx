...
For tools with custom business logic, data transformations, or multi-step workflows, the TypeScript functions framework (Speakeasy Functions) creates serverless functions exposed as tools. Unlike OpenAPI-sourced tools, which wrap existing API endpoints, functions run custom code in isolated environments.
...
For the full framework reference — annotations, tags, environment configuration — see [Creating TypeScript tools](/docs/ai-control-plane/connect/sources/functions).
...
- [Learn the full functions framework](/docs/ai-control-plane/connect/sources/functions)
