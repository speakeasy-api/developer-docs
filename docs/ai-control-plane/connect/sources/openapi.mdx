...
When generation completes, all the generated tools are grouped automatically under the name of the API. If no tools were generated, the wizard surfaces the deployment logs to diagnose the spec.
...
Go to **Distribute > MCP** and click **New MCP Server**. On the server's **Tools** tab, add the tools to expose. The tools generated in step 1 make this a quick selection. See [choosing which tools a server exposes](/docs/ai-control-plane/reference/concepts/tool-definitions#choosing-which-tools-a-server-exposes) for the curation concepts.
...
- [Add custom-code tools with TypeScript](/docs/ai-control-plane/connect/sources/functions)
