...
After [uploading our OpenAPI document](/docs/ai-control-plane/connect/sources/openapi) and creating an MCP server, we'll go to the playground to test it. When we ask the AI agent to get deployment history, it will automatically detect that OAuth authentication is required:
