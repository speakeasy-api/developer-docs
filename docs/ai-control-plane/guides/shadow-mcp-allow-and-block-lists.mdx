...
description: "How the two shadow MCP policy postures work, when to pick each, and how allow rules layer on top of them."
...
## Allow rules layer on top

The policy URL list is the bulk instrument, edited in the policy editor. Allow rules are the per-server instrument, edited from the [Shadow MCP page](/docs/ai-control-plane/secure/shadow-mcp) with **Add Allow Rule**, and they write into the same underlying grant set.
...
- [Shadow MCP](/docs/ai-control-plane/secure/shadow-mcp) — the inventory, statuses, and allow rule actions
- [Risk Policies](/docs/ai-control-plane/secure/risk-policies) — the policy editor and its detect, scope, and action steps
