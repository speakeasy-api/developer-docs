...
The SDK version increments automatically in these cases: 

### Generator version changes

Speakeasy compares feature changes between generator versions:

- For multiple changed features, the largest version bump (major, minor, patch) determines the SDK version increment.
...
New SDKs start at version `0.0.1`. Speakeasy only automatically bumps major versions after reaching `1.0.0`. Breaking changes trigger automatic major version bumps after `1.0.0`.

Major version bumps affect the Golang SDK migration path.
