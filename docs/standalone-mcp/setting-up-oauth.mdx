...
<Callout title="Implementation Support" type="info">
    For assistance with OAuth implementation, including proxy setup and DCR compliance, [reach out to Speakeasy support](https://calendly.com/sagar-speakeasy/30min) for white-glove service.
</Callout>
...
Speakeasy bridges this gap by supporting multiple authentication approaches, from simple token-based methods to complex OAuth proxy solutions. How you set up authentication depends on the OAuth capabilities of your underlying API and the intended purpose of your MCP server. Understanding these different approaches and their trade-offs is essential for selecting the right authentication strategy.
...
If the API is already configured to support DCR, enabling the Authorization Code Flow on Speakeasy is simple:

- Create a manifest file for the OAuth server in Speakeasy.
- Attach the manifest to your toolset.

### Without DCR

Because most APIs don't support DCR, Speakeasy offers an OAuth proxy that translates between MCP requirements and standard OAuth implementations. The proxy uses a specific client ID and secret to access the API on behalf of the MCP server end users.
...
If you want to implement an OAuth proxy in Speakeasy, please [book time with our team](https://calendly.com/sagar-speakeasy/30min). We'll get you up and running.

## Using the Client Credentials grant

The Client Credentials grant is a simpler authentication method. The server exchanges a client ID and secret for access tokens. Speakeasy handles the token exchange process automatically.

Implement the Client Credentials grant in Speakeasy as follows:

- Upload the OpenAPI document to Speakeasy
...
Implement access token authentication in Speakeasy as follows:
