...
## Desktop Extension (DXT) Generation

Speakeasy automatically generates [Anthropic Desktop Extensions (.dxt)](https://github.com/anthropics/dxt) as part of standalone MCP generation. Desktop Extensions provide a packaged format for distributing MCP servers that can be easily installed and managed in Claude Desktop and other MCP-compatible clients.

### Automatic DXT Creation

When generating an MCP server, Speakeasy automatically creates a DXT package alongside your TypeScript server code. The DXT includes:
...
### DXT Manifest Customization

You can customize the generated DXT manifest through the `gen.yaml` configuration file using the `mcp-typescript.dxtManifestOverlay` key. This follows the [DXT Manifest specification](https://github.com/anthropics/dxt/blob/main/MANIFEST.md).
...
### DXT Manifest Population

The DXT manifest is automatically populated using a convention-over-configuration approach:

- **Tool definitions**: Automatically inferred from OpenAPI operations
- **Parameters**: Derived from OpenAPI parameters and request schemas  
- **Descriptions**: Extracted from OpenAPI operation summaries and descriptions
- **Icon**: Automatically detected by searching for `.png` files in your project
- **Metadata**: Pulled from your `gen.yaml` configuration and OpenAPI info section

This ensures the DXT always remains synchronized with your generated MCP server, maintaining a shared source of truth between the server implementation and its distribution package.
...
The easiest way to test your MCP server locally is to use the generated DXT package. Claude Desktop and other MCP clients can load DXT files directly:
...
You can also configure your MCP server to run locally using traditional MCP configuration:
...
Speakeasy generates MCP servers with multiple distribution options:

### Desktop Extension (DXT) Distribution

The generated DXT package provides the most user-friendly distribution method:

- **Easy installation**: Users can install DXT files directly in compatible MCP clients
- **Self-contained**: All dependencies and metadata are bundled together
- **Automatic updates**: DXT format supports version management and update notifications
- **Cross-platform**: Works across different operating systems and MCP clients

### Traditional Distribution

MCP servers can also be published and distributed as npm packages or used directly with `npx`:
...
You can distribute your MCP server using both methods simultaneously, giving users the flexibility to choose their preferred installation method.
