...
Generally an API is not meant to be a database-over-HTTP, so if it feel like it
is starting to recreate SQL or some other query language, it might be going down
the wrong path. There are databases which can be used over HTTP that do not
require creation of a database, like FaunaDB, Firebase, or DynamoDB, which might
be a better fit.
