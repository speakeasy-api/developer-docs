...
To generate and self-host an MCP server from an existing API, see [self-hostable MCP servers with the Speakeasy CLI](/blog/release-standalone-mcp) and [standalone MCP server generation](/product/mcp-server).
