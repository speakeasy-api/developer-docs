...
- Irreversible side effects without confirmation. Use [sampling](/mcp/core-concepts/sampling) for validation if needed.
...
MCP tools are often confused with [MCP resources](/mcp/core-concepts/resources), which are named, read-only data references addressable via URIs. Learn more about the difference in the Resources vs Tools section of the [MCP resources documentation](/mcp/core-concepts/resources#resources-vs-tools).
