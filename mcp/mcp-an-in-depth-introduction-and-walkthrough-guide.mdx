...
|----------------|---------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
| **Host**       | App that holds the LLM and user UI                | [Claude desktop](https://claude.ai/download), [Cursor](https://www.cursor.com/), [Cline](https://cline.bot/), Windsurf  | Generates tool calls, mediates approvals               |
| **MCP Client** | Library embedded in the host                      | [The MCP SDK](https://modelcontextprotocol.io/introduction)                                                             | Maintains a _stateful_ session per MCP Server          |
| **MCP Server** | Lightweight wrapper in front of a tool            | [Sentry MCP](https://github.com/getsentry/sentry-mcp), [GitHub MCP Server](https://github.com/github/github-mcp-server) | Exposes tools and resources uniformly                  |
| **Tool**       | A callable function                               | `list_organizations`, `create_issue`                                                                                    | Discoverable by the client at runtime; typed JSON args |
...
| **Transport**  | How bytes flow between the client and server      | `stdio` or `SSE`                                                                                                        | Local or cloud; always JSON-RPC 2.0                    |
...
| Component                          | Role                                                                                                                             |
|------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| **Cloudflare Worker (`bug-demo`)** | Emits a `TypeError` on every request and reports it to Sentry via the Sentry SDK.                                                |
| **Sentry MCP (hosted)**            | Tools: `list_issues` hosted by Sentry, SSE transport with OAuth                                                                  |
| **GitHub MCP (local)**             | Tools: `list_issues`, `create_issue`, `add_issue_comment`, `create_pull_request` (run with Docker), stdio transport with API key |
...
| ----------------------------------------------------------------------------------- | ----------------- | ---------------------------------- |
| [Node](https://nodejs.org/en/download)                                              | ≥ 18              | To build the Worker and run npx helpers |
| [Docker](https://docs.docker.com/get-started/get-docker/)                           | Latest desktop    | To run the GitHub MCP Server locally  |
| [VS Code](https://code.visualstudio.com/)                                           | Latest desktop    | Code editor                        |
| [Cline](https://marketplace.visualstudio.com/items?itemName=saoudrizwan.claude-dev) | Latest release    | MCP host/agent                   |
| [Sentry account](https://sentry.io/)                                                | Free plan is fine | To catch our crash                 |
| [GitHub account](https://github.com/)                                               | Any tier          | Access token for MCP Server        |
| [Cloudflare account](https://www.cloudflare.com/)                                   | Free plan is fine | To host our buggy code                |
...
![Cloudflare Worker error page displaying TypeError exception](./assets/in-depth-intro/cloudflare-error.png)
...
In VS Code, with Cline installed, follow the steps below:
    ...
    ![Cline VS Code extension showing the MCP Servers configuration panel](./assets/in-depth-intro/cline-setup.png)
...
![Sentry OAuth authorization page for Remote MCP Server access](./assets/in-depth-intro/sentry-auth.png)
...
![Configuring the GitHub MCP server in Cline's settings panel](./assets/in-depth-intro/github-setup.png)
...
![Sentry error details showing the stack trace and TypeError](./assets/in-depth-intro/sentry-error-details.png)
...
![GitHub issue created about the Sentry error](./assets/in-depth-intro/github-issue.png)
...
![GitHub PR showing the bug fix about Sentry issue](./assets/in-depth-intro/github-pr.png)
...
![Fixed Cloudflare Worker successfully serving Hello World response](./assets/in-depth-intro/fixed-worker.png)
