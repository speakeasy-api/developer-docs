...
```bash
git clone https://github.com/ritza-co/speakeasy-examples
cd speakeasy-examples/fastmcp-speakeasy-project/base
```
...
Our project already have OpenAPI document files, such as `openapi.yaml` and `openapi.json`. Therefore, we won't need to perform any configuration on that side. 
