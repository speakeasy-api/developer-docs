...
Because Model Context Protocol (MCP) servers enable communication with APIs, they allow a range of possible integrations, such as users being able to order food or products via Claude Desktop or developers being able to understand your API using Cursor AI.
...
Inside the `mcp_server.py` file, add the below code. This is how you register your FastAPI app with FastMCP using a single line. From there, FastMCP inspects the app, finds the declared controllers in `app/main.py`, and turns each route into an MCP tool without needing any additional configuration. 
...
There's no need to write the configuration yourself. Go into the `app` directory and run the `fastmcp install` command to handle the installation:
...
The `--with` option lets you specify the dependencies to use when running the file. However, `fastmcp` is not the only dependency; we are also working with `fastapi`. You may encounter issues with the Python path, which can result in import errors. Modify the configuration by adding the `fastapi[all]` dependency and an environment variable to set the Python path to ensure Claude can run the project without issues.
...
The integration of FastAPI and FastMCP is now complete. Test the MCP server in the Claude desktop app.
...
You can then ask Claude to create a burger in the chat. In the screenshot below, you can see that Claude was able to make a tool call to create a burger from our API.
...
This command will prompt for authentication. Then, after successful authentication, enter the following path to the OpenAPI document: `./openapi.yaml`.
