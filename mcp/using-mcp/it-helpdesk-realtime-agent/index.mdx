...
We'll use OpenAI's [Realtime Agent SDK](https://platform.openai.com/docs/agents/realtime) for the voice agent. You can find the demo app [here](https://github.com/ritza-co/taskmaster-realtime-agent-demo).
...
This guide uses our [Realtime voice agent demo](https://github.com/ritza-co/taskmaster-realtime-agent-demo) that connects to your Gram-hosted MCP toolset. The demo uses [OpenAI's Realtime Agent SDK](https://platform.openai.com/docs/agents/realtime) to handle both voice interaction and tool calling directly.
...
This same approach works for other IT workflows too. Think about adding password resets from your identity provider, or letting people request software licenses through voice. The pattern stays the same: pick the right APIs, curate the tools, connect them through Gram, and let people talk to them naturally.
