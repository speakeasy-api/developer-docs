...
1. First upload the HubSpot Tickets OpenAPI document and name it "HubSpot Tickets".
   ...
   **Important**: Name the server exactly "HubSpot Tickets" as this affects the environment variable naming convention. The server name gets converted to `HUBSPOT_TICKETS_BEARER_AUTH` in the environment variables.
...
### Create and curate the "Tech Support" toolset
...
When creating your Tech Support toolset, be selective about which tools to enable:


**Only enable these specific tools:**

From `HubSpot` we'll enable the tools for:
- Get ticket by ID (`get_crm_v3_objects_tickets_ticket_idb3ff2d6e`)
- Find tickets by contact email (`post_crm_v3_objects_tickets_search_do_search`)
- Read status/assignee/summary fields (`get_crm_v3_objects_tickets_get_page`)

![Enable HubSpot tools](./assets/hubspot-tools-enable.png)

From `Slack` we'll enable the tools for:
- Fetching Conversations (`conversations_list`)
- Fetching Conversations by ID (for getting a full thread) (`conversations_info`)
...
Configure authentication for both HubSpot and Slack by setting up the environment variables through the UI:

1. Navigate to **Environments** in the sidebar
2. Enter the **Default** environment
3. Click the three dots next to the **Add Variable** button in the top right
4. Click **Fill for toolset**
5. Select your IT Helpdesk toolset from the dropdown
6. Click **Fill variables**
7. Populate the variables with your actual keys:
   - `HUBSPOT_TICKETS_BEARER_AUTH`: Your HubSpot Private App token
   - `HUBSPOT_TICKETS_SERVER_URL`: `https://api.hubapi.com`
   - `SLACK_SERVER_URL`: `https://slack.com/api`
...
Make the server public by clicking the **Public** button and toggling server privacy to public.
