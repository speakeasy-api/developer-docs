...
<Callout title="Running MCP use cases in production?" type="info">
...
Windsurf took a different approach with its IDE. Windsurf Wave 3 includes MCP support, allowing [Cascade](https://codeium.com/blog/introducing-cascade) to connect to multiple servers simultaneously. The configuration is straightforward:
...
The [Solscan MCP](https://github.com/Valennmg/solscan-mcp) server takes a different approach. It's built on [Solscan's](https://solscan.io/) API infrastructure, giving you access to richer data analysis capabilities, like token metadata (names, symbols, logos), market data from various exchanges, holder distribution analytics, and detailed DeFi activity tracking. The server can tell what tokens a wallet holds, how those holdings have changed over time, and which DeFi protocols the wallet has interacted with.
...
The MCP ecosystem has grown to tens of thousands of community-built servers, creating powerful network effects.
...
For example, an MCP server can support environment-specific tool access, exposing separate `staging` and `production` environments, each with its own credentials and server URL tailored to different access requirements.
...
To learn more about MCP and how to try it out, check out our [quickstart guide to installing MCP servers](/mcp/using-mcp/installing-mcp-servers).

Rolling MCP out across an enterprise? See the [Uber enterprise AI playbook](/blog/uber-enterprise-ai-playbook) on building (vs buying) an agent stack, and how to run an [agent-native vendor evaluation](/blog/agent-native-vendor-evaluation).
