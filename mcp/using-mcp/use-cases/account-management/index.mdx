...
![A screenshot of the HubSpot contact record with the "schedule call ASAP" note](assets/hubspot-note-urgent.png)
...
You can run the example service included in our example repository.You can find it at `https://github.com/ritza-co/salesops-taskmaster-tooll`.

After cloning the repo, navigate to the `internal-taskmaster-tool` directory and run the following commands:
...
![assets/gram-salesops-internal.mp4](assets/gram-salesops-internal.mp4)
...
In your SalesOps toolset's MCP tab in Gram, copy the managed auth configuration.

![assets/copy-managed-auth.png](assets/copy-managed-auth.png)
...
To test the complete workflow, open Claude Desktop and send:
