...
- A working understanding of [MCP transports](/mcp/core-concepts/transports) and how the MCP server receives inputs and sends outputs
- Node.js 22 or later
...
- Use a managed platform. Speakeasy's [MCP gateway](/product/mcp-gateway) and [AI control plane](/product/ai-control-plane) generate, host, and govern MCP servers from an OpenAPI document, so you don't run the infrastructure yourself.
...
A managed platform removes most of that operational overhead. Speakeasy's [AI control plane](/product/ai-control-plane) hosts your MCP servers behind one governed entry point with SSO, RBAC, and audit; other options such as [FastMCP Cloud](https://gofastmcp.com/deployment/fastmcp-cloud) exist too.

### Set up OAuth authentication

Implement OAuth 2.1 to secure your remote MCP server. For the current authorization model and a walkthrough, see [MCP authorization](/mcp/securing-mcp-servers/authorizing-mcp-servers).
...
- **Publish your MCP server** on the official Anthropic MCP registry.

For more on distribution, see [why every MCP server needs an install page](/blog/every-mcp-server-needs-an-install-page) and [building an internal MCP registry](/blog/release-internal-mcp-registry).
