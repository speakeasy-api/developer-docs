...
description: How OAuth works in MCP, why Dynamic Client Registration is the hard part, and how the OAuth proxy pattern bridges the gap.
---
...
<Callout type="default">
  The [June 2025 revision of the
  specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization)
  settled how discovery works: the MCP server is an OAuth resource server that
  publishes `/.well-known/oauth-protected-resource` (RFC 9728) and returns it
  via the `WWW-Authenticate` header on a `401`. The client reads that document
  to find the *separate* authorization server, then fetches the authorization
  server's own `/.well-known/oauth-authorization-server` metadata (RFC 8414).
  Current MCP clients follow this flow, but you'll still find older servers and
  clients that expected the MCP server to be its own authorization server.
</Callout>
...
The specification splits the requirements across the two roles. Your MCP server (the resource server) must publish Protected Resource Metadata (RFC 9728), and clients must send the RFC 8707 `resource` parameter. Your authorization server needs to support:

- OAuth 2.1 with mandatory PKCE[^2]
- Authorization Server Metadata (RFC 8414)
- **Dynamic Client Registration (DCR)**[^3], which clients and authorization servers _should_ support

That last item, Dynamic Client Registration, is where the friction usually starts. It's a recommendation rather than a hard requirement, but without it clients can't onboard automatically, and support for it is uneven across OAuth providers.
...
But here's the thing, and it cannot be stressed enough: **many OAuth providers still don't support Dynamic Client Registration** (and yours may not either).

Support is improving as MCP adoption grows, and some providers and identity platforms (such as WorkOS and Auth0) now offer it, but the big consumer identity providers you'd reach for first have historically not. Google, GitHub, and Microsoft Entra ID (formerly Azure AD) have all lacked open DCR, and your company's homegrown OAuth solution that Bob from IT built five years ago almost certainly doesn't have it either.
...
The June 2025 revision of the specification settled the biggest architectural question by making the MCP server a plain OAuth resource server that delegates to a separate authorization server, discovered through Protected Resource Metadata (RFC 9728) and bound to its audience with Resource Indicators (RFC 8707). That removes the ambiguity about whether your MCP server should also be an authorization server: it shouldn't.

What the specification can't fix on its own is uneven Dynamic Client Registration support across existing OAuth providers, which is why the OAuth proxy pattern is still common. As more identity platforms add DCR and ship MCP-aware authorization servers, expect the proxy to fade for greenfield builds. For now, teams implementing MCP still need to weigh how much of the flow they run themselves against what they can delegate to a provider that already speaks the spec.
