...
To demonstrate this principle in action, we created a practical test using the [Dog CEO API](https://dog.ceo/) exposed as MCP servers. The results clearly show how tool count affects LLM performance.
...
We then generated a single MCP server with all 107 tools from that OpenAPI document, and installed this remote MCP server in Claude Desktop.
