...
Under the current specification, your MCP server plays the role of an OAuth *resource server*: it validates tokens and advertises a *separate* authorization server through Protected Resource Metadata, rather than issuing tokens itself. For the full model, including audience-bound tokens and the well-known discovery endpoints, see [MCP authorization](/mcp/securing-mcp-servers/authorizing-mcp-servers).
...
For a deeper treatment of these threats, see our guides to [prompt injection](/resources/prompt-injection) and [tool poisoning attacks against MCP servers](/resources/mcp-tool-poisoning), along with the [NSA's MCP security baseline](/blog/nsa-mcp-security-baseline). To govern MCP servers across an organization, see [shadow MCP detection](/blog/release-shadow-mcp-detection), [agent compliance](/resources/agent-compliance), and how an [AI control plane](/resources/ai-control-plane) enforces these controls at runtime through the [MCP gateway](/product/mcp-gateway).
