...
description: "A guide to building an MCP server for your FastAPI API using FastMCP and Speakeasy and a comparison of the two tools' maintainability, code quality, and usage."
---
...
Open Claude Desktop and click on the **Search and tools** button.
...
Once the installation is complete, reload Claude Desktop. You should see the server and its available tools.
...
You would typically run FastMCP locally inside Claude Desktop. FastMCP works well for local tools, but it's not built for external packaging or reuse across environments.
