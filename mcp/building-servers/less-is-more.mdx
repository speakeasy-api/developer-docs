...
To demonstrate this principle in action, we created a practical test using the [Dog CEO API](https://dog.ceo/) with [Gram](/product/gram)-hosted MCP servers. The results clearly show how tool count affects LLM performance.
...
We then uploaded the OpenAPI document to [Gram](/product/gram) and created a single MCP server with all 107 tools, and installed this remote MCP server in Claude Desktop.
